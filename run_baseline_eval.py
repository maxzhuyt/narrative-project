#!/usr/bin/env python3
"""
Baseline evaluation: same pipeline as run_endings_scaled.py but for
stories in BASELINE_CORPUS/ with b00XX IDs.

Results go to BASELINE_CORPUS/results/ alongside the stories.
"""
...
# ---------------------------------------------------------------------------
# Story discovery — finds b00XX.txt files
# ---------------------------------------------------------------------------

def discover_stories(data_dir):
    """Find all b*.txt files in BASELINE_CORPUS/."""
    stories = {}
    for fname in sorted(os.listdir(data_dir)):
        if not fname.endswith(".txt") or not fname.startswith("b"):
            continue
...
# ---------------------------------------------------------------------------
# Prompts (identical to run_endings_scaled.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sentence splitting (identical to run_endings_scaled.py)
# ---------------------------------------------------------------------------
...
def main():
    ...
    print(f"Discovered {len(stories)} baseline stories")
    for sid in sorted(stories):
        with open(stories[sid]) as f:
